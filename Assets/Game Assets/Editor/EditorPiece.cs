...
namespace StarBattles{ 
    public class EditorPiece : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
    {
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            if (didDrag == false)
            {
                EditorDropPanel edp = GameObject.Find("EditorView").GetComponent<EditorDropPanel>();
                if (linkMode)
                    edp.editorPieceClicked(this);
                else
                {
                    if(edp.selectedEditorPiece != null)
                        edp.selectedEditorPiece.hideAndUnselectJoinPoints();
                    edp.editorPieceClicked(this);
                    showJoinPoints();
                }
            }
